...
sequenceDiagram
    autonumber
    participant App as Caller (UI/C#)
    participant Open as magic_ffplay_open
    participant RT as read_thread
    participant DT as decoder threads
    participant Cond as prep_cond
    App->>Open: open(path)
    Open->>Open: SDL_Init + new FfplaySharedGpu + new VideoState
    Open->>RT: SDL_CreateThread(read_thread)
    Open->>Open: SDL_CreateThread(refresh_thread)
    Open->>Cond: lock + while(prepared==0) wait
    Note over Open: caller BLOCK ở đây
    RT->>RT: avformat_open_input
    RT->>RT: avformat_find_stream_info
    RT->>DT: stream_component_open(video) → spawn video_thread
    RT->>DT: stream_component_open(audio) → spawn audio_thread + SDL_OpenAudioDevice
    alt success
        RT->>Cond: prepared=1, signal
    else lỗi (file hỏng, codec unsupported)
        RT->>Cond: prepared=-1, signal
    end
    Cond-->>Open: unblock
    alt prepared > 0
        Open-->>App: handle (Duration/VideoSize đã valid)
    else prepared < 0
        Open->>RT: abort_request + stream_close
        Open-->>App: nullptr
    end
```

Sơ đồ trên cho thấy *vì sao* hand-shake này quan trọng: caller block trên `prep_cond` thay vì spin-polling `Duration`. Khi điểm 9 hoặc 10 xảy ra, mọi field metadata (codec params, container duration, video size, sample rate) đã được FFmpeg fill, decoder thread đã chạy nền. Caller cầm handle là dùng được ngay — không cần `await` thêm hay retry.

Một điểm tinh tế: `read_thread` đã tự spawn ở bước 3 và `refresh_thread` cũng đã chạy. Nếu user gọi `magic_ffplay_close` đúng lúc đang block (bước 4 đợi tín hiệu), `decode_interrupt_cb` poll `abort_request` cho phép `avformat_open_input` bail giữa chừng — bước 9 sẽ set `prepared = -1` và caller unblock với `nullptr`. Không stuck trên network seek chậm.
...
```mermaid
flowchart TD
    Start([loop entry]) --> Abort{abort_request?}
    Abort -- yes --> Exit([thread exit])
    Abort -- no --> Pause{paused đổi?}
    Pause -- yes --> PauseAct[av_read_pause / av_read_play]
    Pause -- no --> Seek{seek_req?}
    PauseAct --> Seek
    Seek -- yes --> SeekAct[avformat_seek_file<br/>flush audioq + videoq<br/>++serial cả 2 queue]
    Seek -- no --> BP{"audioq+videoq > 15MiB<br/>OR enough_packets cả 2?"}
    SeekAct --> BP
    BP -- yes --> Wait["SDL_CondWaitTimeout<br/>continue_read_thread, 10ms"]
    Wait --> Start
    BP -- no --> Read[av_read_frame]
    Read --> ReadOk{ok?}
    ReadOk -- "EOF / error" --> Null[push null packet<br/>audioq+videoq<br/>eof=1, wait 10ms]
    Null --> Start
    ReadOk -- ok --> SpeedDrop{"speed > 50<br/>AND !keyframe<br/>AND video stream?"}
    SpeedDrop -- yes --> Drop[av_packet_unref<br/>frame_drops_early++]
    Drop --> Start
    SpeedDrop -- no --> Put[packet_queue_put<br/>audioq hoặc videoq<br/>theo stream_index]
    Put --> Start
```

Sơ đồ làm rõ ba "nút" có thể bỏ qua `av_read_frame` (nodes Wait, Null, Drop) — đó là ba cơ chế bảo vệ khác nhau:

- **Wait**: ngăn memory bloat. Cond var `continue_read_thread` được signal từ ba nguồn — decoder kéo packet (giảm `audioq.size`/`videoq.size`), `magic_ffplay_set_speed` (báo có thể đã đổi target), `stream_seek` (báo có công việc khẩn). Nhờ vậy demuxer "thức" đúng lúc cần làm việc, không spin lãng phí CPU.
- **Null**: EOF không phải lỗi, là tín hiệu. Push một packet `data = NULL, size = 0` xuống decoder để FFmpeg flush internal buffer (B-frame còn nằm chờ reorder). Nếu thiếu bước này, frame cuối cùng của file sẽ không bao giờ ra `pictq`.
- **Drop tầng 1**: đặt ở đây thay vì sau decoder vì decoder không thể chạy nhanh hơn ~1500fps; nếu queue lên packet không-keyframe ở 100x mà decoder không nuốt kịp, queue đầy → demuxer block → audioq starve (vì cả audio và video chia chung `MAX_QUEUE_SIZE` byte budget).
...
```mermaid
flowchart LR
    subgraph Producer["audio_thread (decode)"]
        AF[AVFrame PTS=t0] --> SQ[(sampq)]
    end
    subgraph CB["sdl_audio_callback (OS thread)"]
        SQ --> Dec[audio_decode_frame<br/>set audio_clock = PTS + consumed]
        Dec --> Filt{speed != 1.0?}
        Filt -- yes --> AT[atempo / asetrate chain]
        Filt -- no --> Plain[plain copy]
        AT --> Mix[volume / mute mix]
        Plain --> Mix
        Mix --> Out[(stream buffer<br/>~audio_hw_buf_size)]
    end
    Out --> SDLBuf[(SDL internal<br/>double-buffer)]
    SDLBuf --> Drv[(driver buffer)]
    Drv --> DAC[("DAC actual<br/>(sample đang phát)")]

    Dec -. "set_clock_at(audclk,<br/>audio_clock - delay × speed,<br/>callback_time)" .-> AC[audclk]
    DAC -. PTS thực tế .-> AC
```

Để thấy vì sao phải trừ `buffer_delay`, hình dung timeline ngang:
...
thời gian thực ────────────────────────────────────►
              ...
              ┌──────────────────┬──────────────────┐
DAC actual:   │ sample PTS = T0  │ sample PTS = T1  │ ...
              └──────────────────┴──────────────────┘
                                  ▲
                                  │ buffer_delay = ~50-100ms
                                  │ (2×hw_buf + write_buf) / bytes_per_sec
                                  ▼
              ┌──────────────────┬──────────────────┐
audio_clock:  │ vừa decode xong  │ vừa decode xong  │ (đã consume)
              └──────────────────┴──────────────────┘
                                                    ▲
                                                    │ audio_clock biến này
```

Nếu naive set `audclk = audio_clock` → video sẽ "đuổi theo" audio đã được decode/queue nhưng chưa phát ra DAC → video chạy nhanh hơn audio 50-100ms (lipsync sai rõ). Trừ `buffer_delay` để `audclk` về đúng *sample đang phát ra loa*.

Phần `× playback_speed`: ở speed = 2.0, mỗi byte trong SDL buffer ứng với *2 đơn vị thời gian gốc* — vì atempo nén waveform. Buffer 100ms thực tế biểu diễn 200ms PTS gốc. Không nhân `× speed` thì lipsync sẽ lệch tỉ lệ với speed.
...
```mermaid
flowchart LR
    Frame[AVFrame audio<br/>PTS gốc] --> ABS[abuffer src]
    ABS --> S1[atempo=2.0]
    S1 --> S2[atempo=2.0]
    S2 --> S3[atempo=2.0]
    S3 --> S4[atempo=2.0]
    ...
    S5 --> S6[atempo=2.0]
    S6 --> S7[atempo=1.5625]
    S7 --> Sink[abuffer sink<br/>PTS output-time]
    Sink --> Norm["× playback_speed<br/>(đưa PTS về timeline gốc)"]
    Norm --> AC[audio_clock]
```

Sơ đồ cho `speed = 100`: 6 stage `2.0` đẩy tổng từ `1 × 2^6 = 64`, rồi `atempo=1.5625` đẩy lên đúng `64 × 1.5625 = 100`. Mỗi stage nằm trong dải an toàn `[0.5, 2.0]` của OLA, không phát sinh sai số sample đáng kể.

Vì sao không gộp 1 stage `atempo=100` cho gọn? Vì `atempo` ngoài dải `[0.5, 2.0]` mất tính chính xác phase — nội bộ nó nội suy overlap window quá lớn so với chu kỳ tín hiệu → artifact dạng "robot voice" + drift sample đáng kể. Chain nhiều stage nhỏ phải đi qua nhiều `aresample` ngầm, nhưng đó là cost CPU nhỏ; sample count vẫn chính xác.

Lưu ý quan trọng về PTS: Sink trả PTS đã được "time-stretch" (đơn vị output time). Code sau filter nhân `× playback_speed` để mang PTS về timeline gốc (đơn vị giống `vidclk`) trước khi cập nhật `audclk`. Nếu thiếu bước normalize này, ở speed = 100 sau 1 giây thực tế `audclk` chỉ tăng 1/100 giây — A/V đo lệch nhau 99× mà tưởng là sync.
...
```mermaid
flowchart TD
    Tick([tick mỗi 10ms]) --> Empty{"pictq.size > 0?"}
    Empty -- no --> Ret([return])
    Empty -- yes --> Mode{playback_speed == 1.0?}

    Mode -- "yes (audio-master)" --> CTD[compute_target_delay<br/>diff = vidclk - audclk<br/>scale delay theo diff]
    CTD --> Late1{time < frame_timer + delay?}
    Late1 -- yes --> Ret
    Late1 -- no --> Adv1[frame_timer += delay<br/>update_video_pts]
    Adv1 --> LateDrop{"time > frame_timer + next_dur?<br/>(late drop)"}
    LateDrop -- yes --> Drop1[frame_drops_late++<br/>frame_queue_next<br/>retry]
    LateDrop -- no --> Show[mark shown<br/>frame_queue_next]
    Drop1 --> Empty

    Mode -- "no (target-PTS)" --> Wall[wall_elapsed = now - speed_base_wall<br/>target_pts = speed_base_pts + wall × speed]
    Wall --> Loop{next.pts ≤ target?}
    Loop -- yes --> Skip[frame_queue_next<br/>SKIP, không convert]
    Skip --> Loop
    Loop -- no --> Now{current.pts ≤ target?}
    Now -- yes --> Show
    Now -- no --> Ret

    Show --> Display[display block:<br/>VideoProcessorBlt NV12→BGRA<br/>park fp->tex<br/>fire on_frame_ready]
    Display --> Ret
```

Sơ đồ hai nhánh là điểm thiết kế quan trọng nhất của file — hiểu được nó là hiểu được lý do tại sao nhiều editor (CapCut/Premiere) phải tự viết clock thay vì dùng A/V sync chuẩn ở high-speed.
...
sequenceDiagram
    ...
    participant RT as read_thread
    participant VQ as videoq (serial=N)
    participant Dec as video_thread
    participant PQ as pictq
    participant VR as video_refresh

    UI->>RT: stream_seek(pos)<br/>seek_req=1, signal continue_read
    RT->>RT: avformat_seek_file
    RT->>VQ: packet_queue_flush<br/>++serial → N+1<br/>(packet cũ trong queue: pkt.serial=N)
    Note over VQ: queue chứa hỗn hợp:<br/>packet cũ (serial=N)<br/>+ packet mới (serial=N+1)
    RT->>VQ: enqueue flush_pkt (serial=N+1)
    RT->>VQ: enqueue packet mới (serial=N+1)

    Dec->>VQ: get packet<br/>(packet cũ: serial=N)
    Note over Dec: pkt.serial != q->serial<br/>→ skip không decode
    Dec->>VQ: get packet<br/>(flush_pkt: serial=N+1)
    Note over Dec: avcodec_flush_buffers<br/>reset decoder internal
    Dec->>VQ: get packet (serial=N+1)
    Dec->>Dec: decode → AVFrame
    Dec->>PQ: queue_picture (vp.serial=N+1)

    VR->>PQ: peek<br/>(frame cũ: serial=N nếu còn)
    Note over VR: get_clock so c->serial=N<br/>vs *queue_serial=N+1<br/>→ NaN → bỏ qua, không kéo clock
    VR->>PQ: advance đến frame mới (serial=N+1)
    VR->>VR: display, set_clock(N+1)
```

Sơ đồ minh hoạ tinh thần của serial-bump: thay vì **xoá** mọi state cũ ngay lập tức (đòi hỏi lock chéo nhiều thread), ta chỉ tăng một số nguyên `serial`. Mỗi thread khi đụng vào data tự so serial của data với serial hiện tại của queue/clock — không khớp thì coi như "không tồn tại". Bản chất là một dạng *logical clock* để invalidate state lazy, không cần đồng bộ tức thì.

Có ba điểm packet/frame "cũ" có thể tồn tại sau seek, và cả ba đều bị reject bằng cơ chế serial:

1. **Trong videoq/audioq**: packet đã nằm trước flush nhưng decoder chưa kéo ra. Decoder so `pkt.serial != q->serial` → unref và skip.
2. **Trong decoder internal buffer**: B-frame chờ reorder. `flush_pkt` (packet đặc biệt với `data == flush_pkt`) đẩy xuống làm decoder gọi `avcodec_flush_buffers` xoá hết.
3. **Trong pictq/sampq**: frame đã queue_picture nhưng video_refresh chưa show. `get_clock` so `c->serial != *queue_serial` → trả `NaN` → frame không kéo `vidclk` lệch. Frame được `frame_queue_next` qua dần và sẽ bị skip ở display block (`is->paused == false` thì cứ advance).

Toàn bộ cơ chế không cần lock cross-thread — chỉ aligned-int writes atomic theo memory model x86. Nhờ vậy seek không stall thread khác.
...
sequenceDiagram
    autonumber
    participant UI as Caller
    participant SS as set_speed
    participant RT as read_thread
    participant CB as sdl_audio_callback
    participant Filt as audio_filter
    participant VR as video_refresh

    UI->>SS: set_speed(5.0)
    SS->>SS: snapshot speed_base_pts = get_master_clock()<br/>snapshot speed_base_wall_us = now()<br/>playback_speed = clamp(5.0, 0.1, 100.0)
    SS->>SS: speed_changed.store(1)
    SS->>RT: signal continue_read_thread
    SS-->>UI: return (KHÔNG block)

    Note over CB: tick callback tiếp theo
    CB->>CB: speed_changed.exchange(0) → 1
    CB->>Filt: audio_filter_teardown (graph cũ)
    CB->>Filt: audio_decode_frame_filtered<br/>thấy graph==nullptr<br/>→ build chain "atempo=2.0,atempo=2.5"<br/>→ pitch ON (forced vì speed≥5)
    Filt-->>CB: filtered samples
    CB->>CB: set_clock_at(audclk,<br/>audio_clock - delay × 5.0)

    Note over VR: tick refresh tiếp theo
    VR->>VR: speed != 1.0 → target-PTS path<br/>target = speed_base_pts + (now - wall_base) × 5.0
```

Sơ đồ làm rõ tính chất "non-blocking" của `set_speed`: caller chỉ chạy 4 dòng code (snapshot + clamp + atomic store + signal) trong vài chục nano-giây rồi return. Tất cả side-effect đắt (teardown filter graph, rebuild với chain `atempo` mới, alloc swr buffer) đều defer cho audio callback và refresh thread tự lo ở tick kế tiếp.

Hai snapshot ở bước 2 là điểm tế nhị nhất:

- **`speed_base_pts = get_master_clock()`** — chộp PTS hiện tại theo master clock (audclk thường). Đây sẽ là "mốc 0" của target-PTS clock mới.
- **`speed_base_wall_us = av_gettime_relative()`** — chộp wall time đồng thời.

Sau snapshot, refresh thread tính `target_pts = speed_base_pts + (now - speed_base_wall) × speed_mới`. Vì hai snapshot lấy gần như cùng lúc, không có "nhảy" position khi đổi speed — frame tiếp theo show ra sẽ ở đúng vị trí tự nhiên (continuous), chỉ là tốc độ advance khác đi. UX: kéo speed slider từ 1x lên 5x không gây giật ảnh.

Trường hợp đổi speed liên tục (user kéo slider): mỗi call ghi đè snapshot. `speed_changed` là cờ `exchange(0)`, audio callback chỉ teardown một lần ngay cả khi user đổi 10 lần liên tiếp — `exchange` đảm bảo race-free.
...
```mermaid
stateDiagram-v2
    [*] --> Created: CreateTexture2D<br/>(lần đầu)
    Created --> Free: ComPtr giữ 1 ref<br/>refcount = 1
    Free --> InProducer: acquire (refresh_thread)<br/>AddRef → refcount = 2
    InProducer --> InProducerConsumer: UI acquire<br/>AddRef → refcount = 3
    InProducerConsumer --> InProducer: UI release<br/>Release → refcount = 2
    InProducer --> Free: producer recycle<br/>(frame_queue_unref_item)<br/>Release → refcount = 1
    Free --> Free: acquire scan thấy<br/>count == 1 → reclaim
    Created --> Destroyed: pool Reset trên resize<br/>(consumer còn cầm thì sống tiếp đến khi release)
    Destroyed --> [*]
```

Sơ đồ state machine cho thấy refcount đóng vai trò "tag" giữa các trạng thái: 1 = free, 2 = đang được producer hoặc 1 consumer giữ, 3+ = nhiều consumer (rất hiếm). Pool acquire chỉ cần scan 3 entry, dừng ở entry đầu có refcount = 1.

Sự khéo của thiết kế: không cần thêm field `bool isInUse` hay `int useCount` riêng. Refcount đã chính là use count thực sự — mọi `AddRef/Release` của D3D đã atomic và đúng. Pool's permanent ref đảm bảo refcount không bao giờ về 0 (texture không bị destroy đột ngột giữa quá trình lookup).
...
classDiagram
    ...
    class VideoState {
        ...
        Clock extclk
        FfplayGpu* gpu
        atomic uint64* frame_version_seq
        atomic int* speed_changed
        double playback_speed
        double speed_base_pts
        int64_t speed_base_wall_us
        on_frame_ready callback
    }
    class FfplayGpu {
        FfplaySharedGpu* shared
        ID3D11VideoProcessor processor
        ID3D11VideoProcessorEnumerator enumerator
        ComPtr~Texture2D~ bgraPool[3]
        UINT procW, procH
        DXGI_COLOR_SPACE_TYPE inColorSpace
        DXGI_HDR_METADATA_HDR10 hdrMeta
    }
    class FfplaySharedGpu {
        ID3D11Device device
        ID3D11DeviceContext context
        IDXGIDevice dxgi
        ID3D11VideoDevice videoDevice
        ID3D11VideoContext2 videoContext
        mutex bltMutex
        recursive_mutex ffmpegLock
        atomic int refcount
    }
    class FrameQueue {
        Frame queue[16]
        int rindex, windex, size
        int max_size, keep_last
        SDL_mutex mutex
        SDL_cond cond
    }
    class Frame {
    ...
    MagicFFplayHandle "1" o-- "1" VideoState
    VideoState "1" *-- "2" FrameQueue: pictq + sampq
    VideoState "1" *-- "2" PacketQueue: audioq + videoq
    VideoState "1" *-- "3" Clock: audclk + vidclk + extclk
    VideoState "1" *-- "1" FfplayGpu
    FrameQueue "1" *-- "16" Frame
    FfplayGpu "N" --> "1" FfplaySharedGpu : refcount
```

Sơ đồ tóm tắt quan hệ "có" giữa các struct. Hai điểm đáng chú ý:

- **`FfplayGpu` → `FfplaySharedGpu` là N:1 refcounted**. Đây là cốt lõi của chế độ chia sẻ GPU (§1). Mỗi player có `FfplayGpu` riêng (cache VideoProcessor + bgraPool cho cấu hình riêng) nhưng chia sẻ device + context + lock qua `FfplaySharedGpu`.
- **`Clock.queue_serial` trỏ tới `serial` của một PacketQueue (hoặc chính clock đó)**. `audclk.queue_serial = &audioq.serial`, `vidclk.queue_serial = &videoq.serial`, `extclk.queue_serial = &extclk.serial`. Cơ chế này cho phép `get_clock` tự invalidate khi queue flush (§2.9 seek).
...
```mermaid
flowchart TD
    Frame[frame mới] --> S{"sizeChanged?<br/>(W,H đổi)"}
    S -- yes --> Rec[recreate Processor + Enumerator<br/>≈ 1-2 ms]
    Rec --> SCS[SetStreamColorSpace1<br/>SetOutputColorSpace1]
    Rec --> SHDR[SetStreamHDRMetaData]
    Rec --> Done

    S -- no --> C{"colorChanged?<br/>(inCs khác)"}
    C -- yes --> SCS2[SetStreamColorSpace1<br/>SetOutputColorSpace1]
    SCS2 --> M
    C -- no --> M

    M{"metaChanged?<br/>(memcmp HDR10)"}
    M -- yes --> SHDR2[SetStreamHDRMetaData]
    SHDR2 --> Done
    M -- no --> Done

    Done([VideoProcessorBlt])

    SCS --> Done
    SHDR --> Done
```

Cấu trúc 3-tier lazy này quan trọng vì 3 dimension thay đổi với tần suất rất khác nhau:

- **`sizeChanged`**: cực hiếm (vài lần/video, chỉ khi seek qua adaptive bitrate segment khác). Nhưng cost rebuild cao nhất — `CreateVideoProcessorEnumerator` + `CreateVideoProcessor` ~1-2ms.
- **`colorChanged`**: hiếm (vài lần/video, vd HDR PQ section xen kẽ với SDR section). Cost ~50µs.
- **`metaChanged`**: trung bình (HDR10 metadata có thể đổi mỗi keyframe nếu mastering display annotation per-segment). Cost ~10µs.

Triple-cache đảm bảo: 99%+ frame chỉ phải qua 4 phép so sánh (2 int + 1 enum + 1 memcmp 24 byte), không API call nào. Cost dưới 100ns/frame — đo bằng nhiễu thay vì có thể đo trực tiếp.

Nếu naive luôn `SetStreamColorSpace1 + SetStreamHDRMetaData` mỗi frame: mỗi call ~5-10µs (driver phải invalidate caching nội bộ + recompute LUT), 60fps × 4 player = 1.2-2.4ms/giây overhead chỉ cho 2 API call này. Tier hoá → biến mất.
...
```mermaid
flowchart LR
    Disk[(file)] --> RT[read_thread]
    RT -.->|"check<br/>byte > 15MiB<br/>OR packet ≥ 25"| Cond1["cond_wait 10ms<br/>(back-pressure)"]
    Cond1 -.->|"signal khi<br/>decoder consume"| RT
    RT --> AQ[(audioq<br/>~7.5 MiB)]
    RT --> VQ[(videoq<br/>~7.5 MiB)]
    AQ --> AT[audio_thread]
    VQ --> VT[video_thread]
    AT -.->|"sampq full?"| Cond2[cond_wait peek_writable]
    Cond2 -.->|"audio cb consume"| AT
    AT --> SQ[(sampq<br/>9 slot)]
    VT -.->|"pictq full?"| Cond3[cond_wait peek_writable]
    Cond3 -.->|"refresh consume"| VT
    VT --> PQ[(pictq<br/>16 slot)]
    SQ --> SDLC[audio callback]
    PQ --> VR[refresh_thread]
```

Hệ thống có **3 lớp back-pressure** lồng nhau, mỗi lớp giới hạn một loại bloat khác nhau:

1. **Demux → packet queue**: giới hạn bằng *byte* (`MAX_QUEUE_SIZE = 15 MiB`) **kết hợp** *packet count + duration* (`MIN_FRAMES = 25 packet ∧ ≥ 1s duration`). Hai metric đôi vì single metric không đủ:
   - Chỉ byte: stream HEVC 4K có packet rất to (~500KB/I-frame), 15 MiB chỉ ~30 packet ≈ 1s. OK.
   - Chỉ packet count: AAC audio packet rất nhỏ (~700B), 25 packet chỉ ~17KB. Không bảo vệ memory nếu video chiếm hết byte.
   - Kết hợp: lấy ngưỡng nào đạt trước. Bảo vệ cả hai chiều.
2. **Decode video → pictq (16 slot)**: giới hạn theo *slot count*. Decoder block `peek_writable` cho đến khi refresh thread consume. Mỗi slot giữ 1 NV12 hwframe (~3MB cho 1080p) + có thể 1 BGRA texture (~8MB) → 16 slot × ~11MB = ~176MB ceiling cho 1 player 1080p.
3. **Decode audio → sampq (9 slot)**: tương tự, nhỏ hơn vì audio frame nhẹ (~few KB).

Tại sao pictq có 16 mà sampq chỉ 9? Audio không cần lookahead nhiều — `sdl_audio_callback` tick rất đều (mỗi ~10-20ms). Video cần buffer cho A/V sync drift compensation và để absorb GC stall ở UI thread. 16 slot ≈ 250-650ms buffer ở 24-60fps — đủ smooth.
...
```mermaid
flowchart LR
    Dec["decoder vừa output<br/>PTS = audio_clock"] --> WBuf["write buffer<br/>(audio_write_buf_size byte)"]
    WBuf --> HBuf1["SDL primary<br/>(audio_hw_buf_size)"]
    HBuf1 --> HBuf2["SDL secondary<br/>(audio_hw_buf_size)<br/>double-buffer"]
    HBuf2 --> Drv["driver / DAC<br/>sample đang phát<br/>PTS = audio_clock - delay × speed"]

    Dec -. "nếu naive:<br/>audclk = audio_clock<br/>→ video sớm hơn audio<br/>50-100ms" .-> Bad((LIPSYNC SAI))
    Drv -. "đúng:<br/>audclk = audio_clock - delay × speed" .-> Good((SYNC ĐÚNG))
```

Phép trừ `delay × playback_speed` thực ra hỏi: "sample đang phát ra DAC có PTS gốc là bao nhiêu trong timeline của file?" Câu trả lời cần hai bước chuyển đổi đơn vị:

1. **Buffer trong byte → thời gian output (đơn vị clock đã time-stretched)**:
   `delay_output = (2 × hw + write) / bytes_per_sec` giây.
   Đây là thời gian DAC sẽ cần để tiêu thụ hết buffer hiện tại.

2. **Thời gian output → thời gian gốc (PTS units)**:
   `delay_source = delay_output × speed`.
   Vì atempo time-stretch sample, 1 giây output ứng với `speed` giây gốc.

Trừ `delay_source` khỏi `audio_clock` cho ra PTS gốc của sample-đang-phát-thật. Đó chính là điểm video cần align (vidclk dùng PTS gốc, không bị time-stretch).
...
```mermaid
flowchart LR
    subgraph AM["Audio-master (gốc ffplay) — speed=1.0"]
        AC1[audclk]
        AC1 -. "buffer fluctuation<br/>nhỏ ở 1.0x" .-> Sync1[compute_target_delay<br/>diff = vidclk - audclk]
        Sync1 --> Frame1[show frame phù hợp diff]
    end

    subgraph TPC["Target-PTS (custom) — speed != 1.0"]
        Wall[av_gettime_relative<br/>monotonic wall clock]
        Wall --> Calc["target = base_pts +<br/>(wall - base_wall) × speed"]
        Calc --> Frame2[show frame có pts ≤ target<br/>skip frame có pts ≤ target-tiếp]
    end

    style AM fill:#e8f4ff
    style TPC fill:#fff0e8
```

So sánh hai chiến lược qua ví dụ cụ thể *speed = 10x, 30fps source*:

| Mặt | Audio-master ở 10x | Target-PTS ở 10x |
|---|---|---|
| Master source | `audclk` lấy từ atempo output | `av_gettime_relative()` × 10 |
| Jitter | Lớn — SDL deliver không đều ở high-speed (audio cb tốc độ x10 → buffer cạn liên tục) | Gần 0 — wall clock monotonic, không phụ thuộc bất kỳ thread nào |
| Tác động lên video | `diff` swing ±100ms → frame lúc skip 5 lúc show liên tục → giật | Smooth — mỗi tick target tiến đúng `0.0167 × 10 = 167ms`, frame được chọn đều đặn |
| A/V lipsync | Lý thuyết đúng, thực tế jitter ăn vào | Hơi lệch (vài chục ms), nhưng cảm giác xem ổn |

Lý do audio-master "đúng nguyên tắc" nhưng "sai cảm giác" ở high-speed: ở speed 10x, audio decoder và filter graph phải produce sample nhanh gấp 10 lần bình thường. SDL audio cb có chu kỳ cố định (theo `audio_hw_buf_size`), mỗi tick yêu cầu buffer fill toàn bộ. Khi `atempo` chain hơi chậm hoặc thread scheduling delay → `audclk` không advance kịp tốc độ wall × 10, rồi sau đó nhảy bù — gây jitter rõ rệt.

Target-PTS đổi nguồn-of-truth: thay vì hỏi "audio đã chạy đến đâu rồi?" (volatile), hỏi "đã trôi bao nhiêu wall time × speed?" (deterministic). Đánh đổi: nếu audio chậm thật (vd decoder bottleneck), video sẽ chạy trước audio. Nhưng ở speed cao mắt không nhận ra desync vài chục ms; tai cũng không hiểu được audio chi tiết ở 10x — chỉ cần "vẫn có âm thanh phát ra" là đủ.
